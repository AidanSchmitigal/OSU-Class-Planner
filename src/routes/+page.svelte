<script lang="ts">
  ...
  import { Statuses, type Course, Term, Requirements, Requirement } from '$lib';
...
<div class="sticky top-0 z-10">
  ...
  <div class={`flex overflow-x-scroll gap-3 bg-white min-h-[25vh] ${fullScreenList ? 'h-screen max-h-screen' : 'max-h-[50vh]'}`}>
        ...
        <div class="flex gap-1">
          {#each year.terms as term, i}
            <div class="flex-col flex gap-1">
              ...
              <div
                class={`flex p-2 flex-col gap-1 min-w-[6rem] min-h-[10rem] ${year.year === new Date().getFullYear() && Math.floor(new Date().getMonth() / 3) === i ? 'bg-blue-100' : 'bg-gray-50'}`}
...
{#each $selectedPrograms as _program}
  {#each [...$selectedPrograms.flatMap((_program) => [{ data: _program.degree, getter: getDegree, req: Requirement.BACCORE }, { data: _program.college, getter: getCollege, req: Requirement.REQUIRED }, { data: _program.major, getter: getMajor, req: Requirement.MAJOR }, { data: _program.concentration, getter: getConcentration, req: Requirement.CONCENTRATION }]), ...$selectedMinors.map((minor) => ({ data: minor, getter: getMinor, req: Requirement.REQUIRED }))] as program}
    {#if program.data}
          ...
          {#if !remaining.done}
            <div class={`${courseSet.courses.length > 1 ? 'bg-slate-100' : ''} flex flex-col group relative`}>
              ...
              <div class="flex flex-wrap gap-2">
                <CourseList courses={courseSet.courses} psuedoKey={programData?.key + courseSet.label} {dragStart} requirement={program.req} />
              </div>
            </div>
          {/if}
        {/each}
